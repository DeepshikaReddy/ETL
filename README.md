...
1. Try to create dimension tables with integer as Primary key it makes the indexing faster. Also the overall query runs faster with the join condition.
...
3. When creating the foreign keys in fact table use another existing field in dimension which is unique and use that to pull out the primary key in dimension table and insert the value .
4. Use temporary tables whenever possible when doing intermediate steps for data insertion.(# in Synapse and temp keyword in Snowflake)
...
8.	Clustered column store Synapse is the default storage method and the fastest one for large tables.
